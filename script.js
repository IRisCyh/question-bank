const HOMEWORK_ID = "HW1-Momentum";
...
function updateScore() {
  els.score.textContent = `${state.score} / 9`;
}
...
function renderReview() {
  ...
  els.finalScore.textContent = `Multiple-choice score: ${state.score} / 9`;
...
async function submitToTeacher(event) {
  ...
  if (!areAllMultipleChoiceSubmitted()) {
    state.submissionStatus = "Please submit Q1 to Q9 before sending your result to the teacher.";
    renderTeacherSubmission();
    return;
  }
